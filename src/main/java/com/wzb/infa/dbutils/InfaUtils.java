...
public class InfaUtils {
	...
	private Element copyTargetAsDeleteStrategy(Element target, String deleteStrategyName) {
		...
		Element e = null;
		for (@SuppressWarnings("unchecked")
		Iterator<Element> it = target.elementIterator("TARGETFIELD"); it.hasNext();) {
			e = it.next();
	 ...
	 * workflow.addElement("WORKFLOWVARIABLE").addAttribute("DATATYPE",
	 * "integer").addAttribute("DEFAULTVALUE", "") .addAttribute("DESCRIPTION ",
	 * "Status of the previous task that is not disabled") .addAttribute("ISNULL",
	 ...
	 * session.attributeValue("NAME") + ".StartTime") .addAttribute("USERDEFINED",
	 ...
	 * "Status of the previous task that is not disabled") .addAttribute("ISNULL",
	 * "NO").addAttribute("ISPERSISTENT", "NO") .addAttribute("NAME", "$" +
	 * session.attributeValue("NAME") + ".PrevTaskStatus")
	 * .addAttribute("USERDEFINED", "NO");
	 * workflow.addElement("WORKFLOWVARIABLE").addAttribute("DATATYPE",
	 * "integer").addAttribute("DEFAULTVALUE", "") .addAttribute("DESCRIPTION ",
	 * "Error code for this task&apos;s execution").addAttribute("ISNULL", "NO")
	 * .addAttribute("ISPERSISTENT", "NO") .addAttribute("NAME", "$" +
	 * session.attributeValue("NAME") + ".ErrorCode") .addAttribute("USERDEFINED",
	 * "NO"); workflow.addElement("WORKFLOWVARIABLE").addAttribute("DATATYPE",
	 * "string").addAttribute("DEFAULTVALUE", "") .addAttribute("DESCRIPTION ",
	 * "Error message for this task&apos;s execution") .addAttribute("ISNULL",
	 * "NO").addAttribute("ISPERSISTENT", "NO") .addAttribute("NAME", "$" +
	 * session.attributeValue("NAME") + ".ErrorMsg") .addAttribute("USERDEFINED",
	 * "NO"); workflow.addElement("WORKFLOWVARIABLE").addAttribute("DATATYPE",
	 * "integer").addAttribute("DEFAULTVALUE", "") .addAttribute("DESCRIPTION ",
	 ...
	 * workflow.addElement("ATTRIBUTE").addAttribute("NAME", "Save workflow log for
	 * these runs").addAttribute("VALUE", "0");
	 ...
	 * workflow.addElement("ATTRIBUTE").addAttribute("NAME", "Automatically recover
	 * terminated tasks") .addAttribute("VALUE", "NO");
	 * workflow.addElement("ATTRIBUTE").addAttribute("NAME", "Service Level
	 * Name").addAttribute("VALUE", "Default");
	 * workflow.addElement("ATTRIBUTE").addAttribute("NAME", "Allow concurrent run
	 * with unique run instance name") .addAttribute("VALUE", "NO");
	 * workflow.addElement("ATTRIBUTE").addAttribute("NAME", "Allow concurrent run
	 * with same run instance name") .addAttribute("VALUE", "NO");
	 * workflow.addElement("ATTRIBUTE").addAttribute("NAME", "Maximum number of
	 * concurrent runs").addAttribute("VALUE", "0");
	 * workflow.addElement("ATTRIBUTE").addAttribute("NAME", "Assigned Web Services
	 * Hubs").addAttribute("VALUE", "");
	 * workflow.addElement("ATTRIBUTE").addAttribute("NAME", "Maximum number of
	...
	private String getCRC32(Element srcQualifier) {
		...
		Iterator<Element> it = srcQualifier.elementIterator("TRANSFORMFIELD"); it.hasNext();) {
			...
			if ("date/time".equals(e.attributeValue("DATATYPE"))) {
				...
				sb.append(e.attributeValue("NAME"));
				sb.append("||");
			} else if (("binary".equals(e.attributeValue("DATATYPE")))) {
				// binary不支持to_char crc32校验
			} else if (("blob".equals(e.attributeValue("DATATYPE")))) {
				// blob不校验
			} else if (("clob".equals(e.attributeValue("DATATYPE")))) {
				// clob不校验
			} else {
				sb.append("to_char(");
				sb.append(e.attributeValue("NAME"));
	...
	private Element createWorkflow(Element mapping) {
		...
		if ("1".equals(mappingType)) {
			session = createZipperSession(mapping);
		} else if ("2".equals(mappingType)) {
	...
	private Element createAddMapping(Element srcSource, Element tarSource, Element target)
		...
		String owner = srcSource.attributeValue("OWNERNAME");
	...
	public void makeZipperXML(String tableName)
		...
		if (!"-1".equals(mutiParams[0])) {
			...
			for (int i = 0; i < mutiParams.length; i++) {
				e = workflow.createCopy();
				wfNameAdd = mutiParams[i].split(":")[0].trim();
				wfParam = mutiParams[i].split(":")[1].trim();
				// 修改工作流名称
	...
	public void makeAddXML(String tableName)
		...
		Element e = null;
		for (@SuppressWarnings("unchecked")
		Iterator<Element> it = target.elementIterator(); it.hasNext();) {
			e = it.next();
