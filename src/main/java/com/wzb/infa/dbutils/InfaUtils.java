...
public class InfaUtils {
    ...
    private Element createCRC32Expressionfield(Element srcQualifier) {
        ...
        Element crc32Expressionfield = DocumentHelper.createElement("TRANSFORMFIELD")
                .addAttribute("DATATYPE", "string").addAttribute("DEFAULTVALUE", "").addAttribute("DESCRIPTION", "")
                ...
                .addAttribute("PRECISION", "32").addAttribute("SCALE", "0");
	...
	***
        ...
        if ("Source Definition".equals(instance.attributeValue("TRANSFORMATION_TYPE"))) {
            ...
            if ("1".equals(instance.attributeValue("DESCRIPTION"))) {
                sessTransformationInst.addElement("ATTRIBUTE").addAttribute("NAME", "Owner Name").addAttribute("VALUE",
                        infaProperty.getProperty("source.owner", ""));
            }
    ...
    private String getCRC32(Element srcQualifier) {
        ...
        for (@SuppressWarnings("unchecked") Iterator<Element> it = srcQualifier.elementIterator("TRANSFORMFIELD"); it.hasNext();) {
            ...
            if (null == e.attributeValue("DATATYPE")) {
                ...
                switch (e.attributeValue("DATATYPE")) {
                    case "date/time":
                        sb.append("to_char(");
                        sb.append(e.attributeValue("NAME"));
    ...
    private Element createTruncateThenDeleteSession(Element mapping) {
        ...
        Element instance21 = (Element) mapping.selectSingleNode("INSTANCE[@DESCRIPTION='3']");
    ...
    // 创建MAPPING
    private Element createTruncateThenDeleteMapping(Element srcSource, Element target) {
        ...
        connectors.addAll(createConnector(srcQualifierInstance, srcQualifier, targetInstance, target));
    ...
    public void makeZipperXML(String tableName)
        ...
        String owner = infaProperty.getProperty("source.owner", username).toUpperCase();
        String sourceTablename = infaProperty.getProperty("source.prefix", "").toUpperCase() + tableName;
        // 源表名长度大于30时
        if (sourceTablename.length() > 30) {
            sourceTablename = sourceTablename.substring(1, 30);
        }
        ...
        // 目标表名
        String targetTablename = infaProperty.getProperty("target.prefix", "").toUpperCase() + tableName;
        // 目标表名长度大于30时
        if (targetTablename.length() > 30) {
            targetTablename = targetTablename.substring(1, 30);
        }
    ...
    public void makeTruncateThenDeleteXML(String tableName)
        ...
        Element target = copySourceAsTarget(srcSource);
        target.attribute("NAME").setValue(targetTablename);

        // 开发mapping
        Element mapping = createTruncateThenDeleteMapping(srcSource, target);
        // 设置CONFIG
        // Element sessionConfiguration = createSessionConfiguration();
        // 开发work flow
        Element workflow = createWorkflow(mapping);
