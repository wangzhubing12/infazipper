...
public class InfaTable {
    ...
    public String getPkString() throws NoPrimaryKeyException {
        StringBuilder sb = new StringBuilder();
        this.cols.stream().filter((col) -> !("NOT A KEY".equals(col.getKeyType()))).forEachOrdered((col) -> {
            try {
                if (col.getDataType().equals("timestamp") || col.getDataType().equals("date")) {
                    sb.append("to_char(");
                    sb.append(col.getColumnName());
    ...
    public boolean hasCol(String colname) {
        return this.cols.stream().anyMatch((col) -> (colname.equals(col.getColumnName())));
    }
    ...
    public String getColsLenth() {
        int length = 0;
        length = this.cols.stream().map((col) -> Integer.parseInt(col.getPhysicalLength())).reduce(length, Integer::sum);
